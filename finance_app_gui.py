...
class App:
    def __init__(self, root):
        ...
        self.header_lbl = tk.Label(self.root, text="Personal Finance App", height=2)
        self.header_lbl.grid(row=0, column=0, sticky="nsew")
...
class Welcome(tk.Frame):
    ...
    def __init__(self, parent, main_app):
        ...
        self.welcome_lbl = tk.Label(self, text="Welcome to Personal Finance", height=20, bg="blue")
        self.welcome_lbl.grid(row=0, column=0, sticky="nsew")

        self.transaction_btn = tk.Button(self, text="See transactions", bg="orange", command=self.main_app.show_transaction_screen)
        self.transaction_btn.grid(row=1, column=0)
...
class Transactions(tk.Frame):
    ...
    def __init__(self, parent, main_app):
        ...
        # self.scrollbarx.grid(row=current_rows, column=0, columnspan=current_cols, sticky="ew")


        transaction_lbl = tk.Label(self.content_frame, text="Transactions")
        transaction_lbl.grid(row=0, column=0, sticky="nsew")

        self.menu_btn = tk.Button(self.content_frame, text="Main Menu", bg="orange", command=self.main_app.show_welcome_screen)
        self.menu_btn.grid(row=1, column=0, sticky="nsew")

        self.transaction_btn = tk.Button(self.content_frame, text="Query Transactions", bg="orange", command=self.query_transactions)
        self.transaction_btn.grid(row=2, column=0, sticky="nsew")
    ...
    def query_transactions(self):
        ...
        for row_i in range(0, len(transactions)):
